...
{   
    ...
    public IndentAction(int direction) throws IllegalArgumentException
    {
        ...
        addShouldBeEnabledDelegate(new ShouldBeEnabledDelegate()
        {
          public boolean shouldBeEnabled(Action a)
          {
            ...
            HTMLDocument document = (HTMLDocument)ep.getDocument();
            int caret = ep.getCaretPosition();
            
            Element elem = document.getParagraphElement(ep.getCaretPosition());
            
            /* Search if any parent is a list. */
            Element e = HTMLUtils.getListParent(elem);
            /* We can only enable if there are two levels of lists */
            if (e!=null)
            {
              e = HTMLUtils.getListParent(e);
            }
            if (e!=null)
            {
              return true;
    ...
    /** Returns a list of lists and their contents.
     * 
     * @param elems The element list to check.
     * @return A list of Elements representing a list, and each list item
    ...
    /** For each list item create the list.
     * 
     * @param elems
     * @param doc
     */
    ...
    private List<ListItem> getItems(Element list, List selLiElems, int level)
    {
        ...
        for(int i = 0; i < c; i++)
        {
            ...
            if(e.getName().equals(HTML.Tag.LI.toString()))
            {
                ...
                item.listTag = HTMLHelper.getTag(list.getName());
                item.level = level;
    ...
    /** Returns a list of all list elements that have a parent which is
     *  a list type. This seems to be mirrored, not sure why.
     * 
     * @param elems
     * @return
     */
    ...
    private void indent1(ActionEvent e, JEditorPane editor)
    {
        ...
        elems.removeAll(listElems);
        HTMLDocument doc = (HTMLDocument)editor.getDocument();
        // Put a blockquote around all paragraph elements ????? Why?
    ...
    {
        for(Iterator<Element> it = elems.iterator(); it.hasNext();)
        {            
            Element curE = (Element)it.next();            
    ...
    /** Returns all the paragraph elements in the current document 
     *  selection.
     * 
     * @param editor 
     * @return A list of <code>Element</code> representing paragraphs
     *  in the selection.
     */
