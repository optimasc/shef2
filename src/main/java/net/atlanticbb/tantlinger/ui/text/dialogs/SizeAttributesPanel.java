...
public class SizeAttributesPanel extends HTMLAttributeEditorPanel
{
    ...
    public void updateComponentsFromAttribs()
    {
        ...
        if(attribs.containsKey(HTMLHelper.Attributes.HEIGHT)) //$NON-NLS-1$
        {
            heightCB.setSelected(true);
            String h = attribs.get(HTMLHelper.Attributes.HEIGHT).toString(); //$NON-NLS-1$
    ...
    public void updateAttribsFromComponents()
    {
        if(widthCB.isSelected())
        {
            ...
            if(wMeasurementCombo.getSelectedIndex() == 0)
                w += "%"; //$NON-NLS-1$
            attribs.put(HTMLHelper.Attributes.WIDTH, w); //$NON-NLS-1$
        }
        else
            attribs.remove(HTMLHelper.Attributes.WIDTH); //$NON-NLS-1$
        ...
        if(heightCB.isSelected())
        {
            ...
            if(hMeasurementCombo.getSelectedIndex() == 0)
                h += "%"; //$NON-NLS-1$
            attribs.put(HTMLHelper.Attributes.HEIGHT, h); //$NON-NLS-1$
        }
        else
            attribs.remove(HTMLHelper.Attributes.HEIGHT); //$NON-NLS-1$
    }
    ...
    private JCheckBox getHeightCB()
    {
        if(heightCB == null)
        {
            ...
            heightCB.setText(i18n.str(HTMLHelper.Attributes.HEIGHT)); //$NON-NLS-1$
