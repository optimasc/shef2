...
import javax.swing.text.html.XHTMLDocument;
...
public class WysiwygHTMLEditorKit extends HTMLEditorKit
{
  ...
  private KeyStroke tabBackwardKS = KeyStroke.getKeyStroke(KeyEvent.VK_TAB,
  ...
  /**
   * Fetch a factory that is suitable for producing views of any models that are
  ...
  public class WysiwygHTMLFactory extends HTMLFactory
  {
    public View create(Element elem)
    {
      ...
      if (o instanceof HTML.Tag)
      {
        ...
        {
          ResizableView v = new ResizableView(new ImageIconView(elem));
          monitoredViews.add(v);
          return v;
        }
        ...
        /* Address is a block view */
        else if (kind == HTMLHelper.ADDRESS_X)
        {
  ...
  private class ResizeHandler extends MouseInputAdapter
  {
    ...
    /**
     * Updates the list of monitored ResizeableViews. If they don't exist in the
     * document, they're removed from the list.
     * 
     * @param doc
     */
  ...
  private class ResizableView extends DelegateView
  {
     ...
     * 
     * @param p
     * @return one of NW, SW, NE, SE, N, S, E, W or -1 if a handle is not at the
     *         point
     */
   ...
   * which doesn't really work well in practice and tends to confuse users. This
   * view replaces the default and simply draws the elements as blocks that
