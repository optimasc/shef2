...
public class LinkAttributesPanel extends HTMLAttributeEditorPanel
{
    ...
    {
        if(attribs.containsKey(HTMLHelper.Attributes.NAME)) //$NON-NLS-1$
        {
            nameCB.setSelected(true);
            nameField.setEditable(true);
            nameField.setText(attribs.get(HTMLHelper.Attributes.NAME).toString()); //$NON-NLS-1$
        }
        else 
        {
        ...
        if(attribs.containsKey(HTMLHelper.Attributes.TARGET)) //$NON-NLS-1$
        {
            openInCB.setSelected(true);
            String val = attribs.get(HTMLHelper.Attributes.TARGET).toString(); //$NON-NLS-1$
    ...
    public void updateAttribsFromComponents()
    {
        if(openInCB.isSelected())
            attribs.put(HTMLHelper.Attributes.TARGET, TARGETS[openInCombo.getSelectedIndex()]); //$NON-NLS-1$
        else
            attribs.remove(HTMLHelper.Attributes.TARGET); //$NON-NLS-1$
        ...
        if(nameCB.isSelected())
            attribs.put(HTMLHelper.Attributes.NAME, nameField.getText()); //$NON-NLS-1$
        else
            attribs.remove(HTMLHelper.Attributes.NAME); //$NON-NLS-1$
    }
