...
public class CellAttributesPanel extends HTMLAttributeEditorPanel
{
    ...
    public void updateComponentsFromAttribs()
    {
        ...
        if(attribs.containsKey(HTMLHelper.Attributes.COLSPAN)) //$NON-NLS-1$
        {
            Integer value = new Integer(attribs.get(HTMLHelper.Attributes.COLSPAN).toString());
            // I don't know why, but value is automatically added in the HTML attributes, do not enable control
            ...
            try
            {
                colSpanField.getModel().setValue(value); //$NON-NLS-1$
            }
            catch(Exception ex)
            {
                ex.printStackTrace();
        ...
        if(attribs.containsKey(HTMLHelper.Attributes.ROWSPAN)) //$NON-NLS-1$
        {
          Integer value = new Integer(attribs.get(HTMLHelper.Attributes.ROWSPAN).toString());
          // I don't know why, but value is automatically added in the HTML attributes, do not enable control
          // if these are the default values.
          if (value.intValue() != 1)
          {
            ...
            try
            {
                rowSpanField.getModel().setValue(value); //$NON-NLS-1$
            }
            catch(Exception ex)
            {
                ex.printStackTrace();
        ...
        if(attribs.containsKey(HTMLHelper.Attributes.BGCOLOR)) //$NON-NLS-1$
        {
            ...
            bgColorPanel.setColor(attribs.get(HTMLHelper.Attributes.BGCOLOR).toString()); //$NON-NLS-1$
        }
        else
        {
        ...
        dontWrapCB.setSelected(attribs.containsKey(HTMLHelper.Attributes.NOWRAP));  //$NON-NLS-1$
    }
    ...
    {        
        ...
        if(dontWrapCB.isSelected())
            attribs.put(HTMLHelper.Attributes.NOWRAP, "nowrap"); //$NON-NLS-1$ //$NON-NLS-2$
        else
            attribs.remove(HTMLHelper.Attributes.NOWRAP); //$NON-NLS-1$
        
        if(bgColorPanel.isSelected())
            attribs.put(HTMLHelper.Attributes.BGCOLOR, bgColorPanel.getColor()); //$NON-NLS-1$
        else
            attribs.remove(HTMLHelper.Attributes.BGCOLOR); //$NON-NLS-1$
        
        if(colSpanCB.isSelected())
            attribs.put(HTMLHelper.Attributes.COLSPAN, colSpanField.getModel().getValue().toString()); //$NON-NLS-1$
        else
            attribs.remove(HTMLHelper.Attributes.COLSPAN); //$NON-NLS-1$
        
        if(rowSpanCB.isSelected())
            attribs.put(HTMLHelper.Attributes.ROWSPAN, rowSpanField.getModel().getValue().toString()); //$NON-NLS-1$
        else
            attribs.remove(HTMLHelper.Attributes.ROWSPAN); //$NON-NLS-1$
    }
