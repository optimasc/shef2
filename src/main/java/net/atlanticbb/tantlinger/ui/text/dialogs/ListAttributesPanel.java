...
public class ListAttributesPanel extends HTMLAttributeEditorPanel
{
    ...
    {
        //updateForType();
        if(attribs.containsKey(HTMLHelper.Attributes.TYPE)) //$NON-NLS-1$
        {
            styleCB.setSelected(true);
            styleCombo.setEnabled(true);
            int i = getIndexForStyle(attribs.get(HTMLHelper.Attributes.TYPE).toString()); //$NON-NLS-1$
        ...
        if(attribs.containsKey(HTMLHelper.Attributes.START)) //$NON-NLS-1$
        {
            startAtCB.setSelected(true);
            startAtField.setEnabled(true);
            try
            {
                int n = Integer.parseInt(attribs.get(HTMLHelper.Attributes.START).toString()); //$NON-NLS-1$
    ...
    public void updateAttribsFromComponents()
    {
        ...
        {
            if(typeCombo.getSelectedIndex() == UL_LIST)
                attribs.put(HTMLHelper.Attributes.TYPE, UL_TYPES[styleCombo.getSelectedIndex()]); //$NON-NLS-1$
            else
                attribs.put(HTMLHelper.Attributes.TYPE, OL_TYPES[styleCombo.getSelectedIndex()]); //$NON-NLS-1$
        }
        else
            attribs.remove(HTMLHelper.Attributes.TYPE); //$NON-NLS-1$
        
        if(startAtCB.isSelected())
            attribs.put(HTMLHelper.Attributes.START, startAtField.getModel().getValue().toString()); //$NON-NLS-1$
        else
            attribs.remove(HTMLHelper.Attributes.START); //$NON-NLS-1$
    }
