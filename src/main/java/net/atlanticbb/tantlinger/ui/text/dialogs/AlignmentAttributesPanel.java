...
public class AlignmentAttributesPanel extends HTMLAttributeEditorPanel
{
    ...
    public void updateComponentsFromAttribs()
    {
        if(attribs.containsKey(HTMLHelper.Attributes.ALIGN)) //$NON-NLS-1$
        {
            ...
            hLocCombo.setSelectedItem(attribs.get(HTMLHelper.Attributes.ALIGN)); //$NON-NLS-1$
        }
        else
        {
        ...
        if(attribs.containsKey(HTMLHelper.Attributes.VALIGN)) //$NON-NLS-1$
        {
            vAlignCB.setSelected(true);
            vLocCombo.setEnabled(true);
            vLocCombo.setSelectedItem(attribs.get(HTMLHelper.Attributes.VALIGN)); //$NON-NLS-1$
        }
        else
        {
    ...
    public void updateAttribsFromComponents()
    {
        if(vAlignCB.isSelected())
            attribs.put(HTMLHelper.Attributes.VALIGN, vLocCombo.getSelectedItem().toString()); //$NON-NLS-1$
        else
            attribs.remove(HTMLHelper.Attributes.VALIGN); //$NON-NLS-1$
        
        if(hAlignCB.isSelected())
            attribs.put(HTMLHelper.Attributes.ALIGN, hLocCombo.getSelectedItem().toString()); //$NON-NLS-1$
        else
